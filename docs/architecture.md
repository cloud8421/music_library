...
**Elixir 1.20.3, Phoenix ~> 1.8, LiveView ~> 1.1, SQLite3**
...
| `TelemetryMetrics`     | (telemetry_datapoints via TelemetryRepo)                              | Read-only telemetry context for the metrics API. Provides `available_metrics/0` (metric descriptors + categories) and `overview/1` (bounded grouped summaries with percentiles). Owns parameter parsing (since, categories, top), indexed queries via `TelemetryRepo`, and compact top-N summarisation. No writes — reads existing persisted telemetry rows only.       |
...
| `TelemetryMetrics.Definitions`            | Shared source-of-truth for telemetry metric definitions, tag helpers (Finch/Oban/Phoenix/LiveView), drop rules, category assignment, and descriptor normalisation. Consumed by `MusicLibraryWeb.Telemetry`, `Storage`, and the metrics API.    |
| `TelemetryMetrics.MetricKey`              | Stable metric key generation (`<struct>:<name>:<tags>`) matching the format historically persisted in `telemetry_datapoints`. Used by `Storage` and the metrics API to ensure consistent key format across reads and writes.                   |
...
| `MetricsController`    | `/api/v1/metrics`, `/api/v1/metrics/overview`                                                                                                                | Read-only JSON API for telemetry metric descriptors and bounded category overviews with since/categories/top filtering (requires Bearer token) |
...
| `prod-metrics`         | `/prod-metrics` | `fetch_production_metrics_overview`                                                                    | Browse and triage production telemetry metrics via metrics API                |
